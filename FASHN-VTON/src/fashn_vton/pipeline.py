...
class TryOnPipeline:
    ...
    def _setup_tryon_model(self):
        ...
        self.tryon_model._dml_final_layer_cpu = (
            copy.deepcopy(self.tryon_model.final_layer)
            .cpu()
            .float()
            .eval()
        )
    ...
    def _sample(
        ...
        *,
        ...
        for step_idx, (t_curr, t_prev) in enumerate(
            ...
            # computing it (forward_for_cfg skips that forward pass
            # entirely, saving a full transformer pass + CPU roundtrip
            # on each of these steps).
            skip_cfg = (
                skip_cfg_last_n_steps > 0
                and step_idx >= num_timesteps - skip_cfg_last_n_steps
            )

            pred = self.tryon_model.forward_for_cfg(
                images, t_vec, compute_uncond=not skip_cfg, **model_kwargs
            )
            v_c = pred["v_c"]
    ...
    def __call__(
        ...
        def prepare_tensor(img: np.ndarray) -> torch.Tensor:
            ...
            # Cast to the inference dtype *before* repeating across
            # num_samples. Repeating first (as fp32) and downcasting
            # afterward means the num_samples-times-larger tensor briefly
            # exists at fp32 size on the device before being converted,
            # and these tensors are kept alive in model_kwargs for the
            # whole sampling loop, not just for one step. Casting first
            # means .repeat() only ever allocates the smaller fp16/bf16
            # buffer, halving the standing VRAM cost of this tensor.
            t = t.to(self.device, dtype=self.inference_dtype)
